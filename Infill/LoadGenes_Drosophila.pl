...
print "Gene Load Ensembl version $version start: ".showtime()."\n";
    ...
    my $genes = $db->get_collection( 'FruitFLYgenes' );
    my $spc = $db->get_collection( 'Species' );    
    $genes->drop;
    my %ensIDs;
my $ga = $reg->get_adaptor('Fruitfly', 'Core', 'Gene');    
my $sa = $reg->get_adaptor('Fruitfly', 'Core', 'Slice');
my $dba = $reg->get_adaptor('Fruitfly', 'Core', 'DBentry');
...
print "$cntG genes loaded \n";
 ...
 # $spc->insert( {
 #    name => "Fruitfly",
 #    aliases => ["Drosophila", "Drosophila melanogaster", "Fruit fly"],
 #    collection => "FruitFLYgenes",
 #    genecount => $cntG,
 #    release => $version
 #  });
