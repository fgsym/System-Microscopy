		...
		if(sidelis[i].childNodes){
			...
			if(  (sidelis[i].childNodes.length>2) && (sidelis[i].childNodes[0].innerHTML!=undefined)   ){
				...
				if(sidelis[i].className != "clickmeopen"){
					...
					if (path.match(/pheno/)) {
					  document.getElementById(path.split("/")[1]).className="clickmeopen";
					  document.getElementById(path.split("/")[1] + ":set").className="clickmeopen";
					  document.getElementById(path.split("/")[1] +"_"+ path.split("/")[2]).className="ahighl";
					}
